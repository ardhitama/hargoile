...
Route simulateMovement(String routeName, int updateInterval, int distance, double speed)
{
	...
	//while(currentDistance < distance)
    {
        if(tick % updateInterval == 0)
        {
            ...
            noise = (hgl::rangedRand(0,50)/10) / 111315.0f; // noise is 0 to 5 meters
            ...
            //gp.id = ++gpCount;
            genRoute.addPoint(gp);
			//currentDistance += Number::geodeticDistance(
        }
        ++tick;
    }

    if(genRoute.getGeoPointData().size() == 1)
    {
        GeoPoint gp(-7.782875f, 110.367098, 60, 60, 0, 0);
        //gp.id = genRoute.getGeoPointData().size();
        genRoute.addPoint(gp);
    }
    return genRoute;
}
