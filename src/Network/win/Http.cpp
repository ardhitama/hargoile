...
void Http::post(HttpRequest& httpRequest)
{
	std::cout << "Sending POST request to " << "\"" << httpRequest.getUri() << "\"\n\n";
	std::cout << httpRequest.getPostContentRequest().data();
}

void Http::get(const String& url)
{
    //
}

void Http::get(const String& url, HttpRequest& param)
{
    //
}
