...
double GeoCalc::vectorDistance(double lat1, double lon1, double alt1, double lat2, double lon2, double alt2)
{
	...
	double N1 = a / sqrt(1 - e2 * pow(sin(lat1), 2)); // radius of curvature in prime vertical
	double N2 = a / sqrt(1 - e2 * pow(sin(lat2), 2));

	// convert lat1, lon1, alt1 to spherical coordinates
	double x1 = (N1 + alt1) * cos(lat1) * cos(lon1);
	double y1 = (N1 + alt1) * cos(lat1) * sin(lon1);
	double z1 = (N1 * (1-e2) + alt1) * sin(lat1);

	// convert lat2, lon2, alt2 to spherical coordinates
	double x2 = (N2 + alt2) * cos(lat2) * cos(lon2);
	double y2 = (N2 + alt2) * cos(lat2) * sin(lon2);
	double z2 = (N2 * (1-e2) + alt2) * sin(lat2);
...
double GeoCalc::greatCircleDistanceHaversine(double lat1, double lon1, double lat2, double lon2)
{
	// ellipsoid defining parameters according WGS84
